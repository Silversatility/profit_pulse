...
export default class ProfileCtrl {
    ...
    populate(){
        this.userService.me()
            .then(data => {
                ...
                if (this.myProfile.user.is_customer == true) {
                  this.profiles.user_type = "customer"
                } else if (this.myProfile.user.is_sales_representative == true) {
                  this.profiles.user_type = "sales_representative"
                } else {
                  this.profiles.user_type = "manager"
                }

                this.profiles.business_name = data.business_name ? data.business_name : null;
                this.profiles.clinic_type = data.clinic_type ? data.clinic_type : null;
                ...
                this.profiles.customer_goal = data.customer_goal;
    ...
    save(){
        let data = {
            ...
            customer_goal: this.profiles.customer_goal,
            overhead: this.profiles.overhead,
            commissions_owed: this.profiles.commissions_owed,
            business_name: this.profiles.business_name,
            clinic_type: this.profiles.clinic_type,
            user_type: this.profiles.user_type,
        };
    ...
    updateProfileManager(userID, data){
        if (data.user_type == "customer") {
          this.$q.all([this.userService.updateCustomer(userID, data), this.changPassword()])
              .then(success => {
        ...
        } else if (data.user_type == "sales_representative") {
          ...
          this.$q.all([this.userService.updateManager(userID, data), this.changPassword()])
            .then(success => {
              this.updateProfileSuccess(success);
            })
            .catch(error => {
