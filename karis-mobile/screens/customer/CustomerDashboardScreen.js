...
import { listCustomerReports, listCustomerTotalProfits } from '../../redux/actions';
import KarisSnackbar from '../../components/KarisSnackbar';
import DateFilter from '../../components/DateFilterDashboard';
    ...
    return (
      <View style={Styles.container}>
        <View style={localStyles.adminReportRow}>
          ...
          <View style={localStyles.adminReportItem}>
            <Text style={localStyles.adminReportItemLabel}>Profit</Text>
            <Text style={localStyles.adminReportItemPrice}>${this.props.data.margin}</Text>
          </View>
        </View>
        <View style={localStyles.divider} />
        <View style={localStyles.adminReportRow}>
          <View style={localStyles.adminReportItem}>
            <Text style={localStyles.adminReportItemLabel}>Drugs Dispensed</Text>
            <Text style={localStyles.adminReportItemPrice}>{this.props.data.dispense_count}</Text>
          </View>
          <View style={localStyles.adminReportItem}>
            <Text style={localStyles.adminReportItemLabel}>Products Dispensed</Text>
            <Text style={localStyles.adminReportItemPrice}>{this.props.data.products_count}</Text>
          </View>
        </View>
      </View>
    );
...
class CustomerTotalProfitsChart extends React.Component {
  render() {
    return(
      <View style={localStyles.chartContainer}>
        <View style={localStyles.chartHeader}>
          <Text style={localStyles.chartHeaderLabel}>Profit</Text>
          <DateFilter
...
class DashboardScreen extends Component {
  ...
  _initialize = async (params = { duration: 'year_to_date' }) => {
    ...
    await this._fetchReports(params);
    await this._fetchTotalProfits(params);

    this.setState({ loading: false });
    // this.setState({ snackbar: { visible: true, message: 'Failed to fetch admin reports' } });
  }

  _fetchReports = async (params) => {
    await this.props.listCustomerReports(params);
  }

  _fetchTotalProfits = async (params) => {
    await this.props.listCustomerTotalProfits(params);
    const chartData = {
      labels: [],
      datasets: [{ data: [] }],
    };
  ...
  render() {
    return (
      <View style={Styles.container}>
        ...
        />
      <CustomerHeader navigation={this.props.navigation} headerTitle='Dashboard' />
        <ScrollView alwaysBounceVertical={false} contentContainerStyle={[Styles.scrollableContainer, Styles.content]}>
          {
            this.state.loading
              ...
              : (
                  <View>
                    <CustomerReports data={this.props.reports.results} />
                      <View style={localStyles.divider} />
                      <CustomerTotalProfitsChart durationFilter={this.props.reports.params.duration} reFetch={this._initialize} data={this.state.chartData}/>
                  </View>
                )
          }
        </ScrollView>
...
const mapDispatchToProps = (dispatch) => {
  return {
    listCustomerReports: (params) => dispatch(listCustomerReports(params)),
    listCustomerTotalProfits: (params) => dispatch(listCustomerTotalProfits(params)),
  };
};
