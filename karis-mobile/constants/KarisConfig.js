...
let localApiBaseURL;
try {
  localApiBaseURL = `http://${Constants.manifest.debuggerHost.split(':').shift()}:8000`;
} catch (error) {
...
const AllConfig = {
  production: {
    ...
    CUSTOMER_PORTAL_URL: 'https://customer.karispharma.com/'
  },
  development: {
    ...
    CUSTOMER_PORTAL_URL: 'https://stage-customer.marcelo.ph/'
  },
  local: {
    ...
    CUSTOMER_PORTAL_URL: 'http://localhost:8001/dist/customer/'
  }
};

let KarisConfig = AllConfig.local;
if (! __DEV__) {
  KarisConfig = AllConfig.production;
}

export default KarisConfig;
