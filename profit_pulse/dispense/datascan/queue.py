...
class DataScanQueueing:
    ...
    def csv_file_exists(cls, file_path):
        return DispenseReport.objects.filter(report_url=file_path).exists()

    @classmethod
    def enqueue_csv_file(cls, file_path):
        return DispenseReport.objects.create(report_url=file_path, status=DispenseReport.STATUS.pending)
